 ...
 * How does BCV affect the treasury balance?
 ...
 Assumptions
 -----------
 Whether a user bonds or stakes is governed by the excel sheet
 "Wonderland (MINT or STAKE)".

Variables
---------
 * ohm_deps: OHM deposits
 ...
 * BCV: Bond Control Variable. The scaling factor at which bond prices change.
                             ...
                             LESS ASSETS ADDED TO TREASURY?).

Questions
---------
ohm_total_supply: IS THIS DIFFERENT FROM OHM_SUPPLY?
                  SHOULD IT BE THE TOTAL SUPPLY OF STAKED OHM OR THE
                  TOTAL SUPPLY OF OHM (STAKED AND NOT STAKED)?
                  IT SEEMS TO BE THE TOTAL SUPPLY, INDEPENDENT OF WHETHER
                  STAKED OR NOT.
                  SEE REWARD YIELD:
                    Reward yield refers to the actual amount of OHM received by
                    each staker on each rebase. The reward yield is a rough
                    target from a policy point of view. It can almost never be
                    maintained precisely due to e.g. fluctuating amounts of OHM
                    staked.
...
n_rebase: Are there 15 rebases in a vesing period? Does vesing payout (redeem)
          only occur at rebases? My impression is that the claimable rewards on
          Wonderland are updated more frequently.

How to model ohm_price?
    - linear increase with a USD per day.
